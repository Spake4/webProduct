  ...
  return (
    <div className="min-h-screen p-6 pt-10">
      <motion.div
        ...
        className="mb-8"
      >
        <span className="text-xs font-semibold tracking-[0.25em] text-violet-400 uppercase">Сообщество</span>
        <h1 className="font-syne font-bold text-3xl lg:text-4xl text-white mt-1.5">
          Галерея <span className="gradient-text">работ</span>
        </h1>
        <p className="text-white/40 text-sm mt-2">Лучшие результаты пользователей платформы</p>
      </motion.div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {data?.items.map((item, idx) => (
              <motion.div
                ...
                whileHover={{ y: -5 }}
                className="group relative aspect-square rounded-2xl overflow-hidden bg-[#16161f] border border-white/5 hover:border-violet-500/30 hover:shadow-[0_0_30px_rgba(124,58,237,0.25)] transition-all duration-300"
              >
                <img
